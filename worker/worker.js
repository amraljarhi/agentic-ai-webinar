...
const EVENT_WHEN = "Thursday, 15 October 2026 · 10:00–11:10 CEST";
...
<div style="display:none;max-height:0;overflow:hidden;opacity:0;color:#f4f5f7;font-size:1px;">You're in. Thu 15 Oct, 10:00 CEST — your Microsoft Teams join link is inside.</div>
...
async function sendConfirmationEmail(env, toEmail, fullName) {
  ...
  const subject = `You're registered: Beyond Copilot (15 Oct, 10:00 CEST) — your Teams link inside`;
  ...
  // for each registration; you review and send it manually. This avoids the tenant
  // mail-flow rule that blocks Power Platform from *sending* to external recipients —
  // creating a draft is not a send, and your manual send from Outlook is a normal user
  // email (not blocked). The flow maps: To = to, Subject = subject, Body = html.
